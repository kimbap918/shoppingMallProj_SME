		...
		<form name="readForm" onsubmit="return false;">
			...
			<div class="form-group" style="display: none">
				<label for="bno">글 번호</label> <input type="text"
					class="form-control2" id="bno" name="bno" value="${boardread.bno}"
					readonly />
			</div>
			...
			<div class="form-group">
				...
				<textarea style="resize: none;" class="form-control" id="content"
					name="content"><c:out value="${boardread.content}" /></textarea>
			</div>

			<div class="form-group">
				<label for="writer" class="col-sm-2 control-label">작성자</label> <input
					type="text" class="form-control3" id="writer" name="writer"
					value="${boardread.writer}" readonly />
			</div>

			<div class="form-group">
				<label for="regdate" class="col-sm-2 control-label">작성날짜</label>
				<fmt:formatDate value="${boardread.regdate}"
					pattern="yyyy-MM-dd hh:mm:ss" />
			</div>

			<div>
				<c:if test="${userInfo.name eq boardread.writer}">
					<button class="update_btn btn-primary" id="boardupdate_btn"
						name="boardupdate_btn">수정</button>
					<button class="delete_btn btn-primary" id="boarddelete_btn"
						name="boarddelete_btn">삭제</button>
				</c:if>
				<button class="boardlist_btn btn-primary"
					onclick=" location.href = 'boardlist.do' ; " name="boardlist_btn">목록</button>
			</div>
		</form>
	</section>
	<hr />
